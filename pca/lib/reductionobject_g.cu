...
__shared__ unsigned int global_object_offset[NUM_THREADS/WARP_SIZE];

__device__ void Reduction_Object_G::oma_init(int *testdata)
{
			...
			if(getLock(&locks[index]))
			{
				...
				if(buckets[index]==0)
				{
					if(kvn==0)	
					{
						//If the bucket is empty, the key has not appeared in the reduction object, create a new key-value pair
						//Copy the real data of key and value to shared memory
						int k  = omalloc(2+key_size);//The first byte stores size of the key
						if(k==-1)//The shared memory pool is full
						ret = false;
...
__device__ unsigned short Reduction_Object_G::get_value_size(unsigned int bucket_index)
{
	unsigned int key_index = ((unsigned int *)&buckets[bucket_index])[0]; 
