...
void preallocBlockSums(unsigned int maxNumElements,
                       int** &g_scanBlockSums,
                       unsigned int &g_numEltsAllocated,
...
//    assert(g_numEltsAllocated == 0); // shouldn't be called 
    ...
    {       
        ...
        if (numBlocks > 1) 
        {
            CE(cudaMalloc((void**) &g_scanBlockSums[level++],  
...
void deallocBlockSums(int** g_scanBlockSums,
                      unsigned int g_numEltsAllocated,
                      unsigned int g_numLevelsAllocated)
{
...
void prescanArray(int *outArray, int *inArray, int numElements)
{
    ...
    preallocBlockSums(numElements,
                      g_scanBlockSums,
                      g_numEltsAllocated,
                      g_numLevelsAllocated);

    prescanArrayRecursive(outArray, inArray, numElements, 0, g_scanBlockSums);
...
int prefix_sum( int* d_inArr, int* d_outArr, int numRecords )
{	
	prescanArray( d_outArr, d_inArr, numRecords );
	int inLast,outLast;
        memcpyD2H (&outLast, d_outArr + numRecords - 1, sizeof(int));
        memcpyD2H (&inLast, d_inArr + numRecords - 1, sizeof(int));
	return outLast + inLast;
}

int prefix_sum( unsigned int* d_in, unsigned int* d_out, unsigned int numRecords )
{	
	int * d_inArr=(int*)d_in;
	int * d_outArr=(int*)d_out;
	prescanArray( d_outArr, d_inArr, numRecords );
	int inLast,outLast;
        memcpyD2H (&outLast, d_outArr + numRecords - 1, sizeof(int));
        memcpyD2H (&inLast, d_inArr + numRecords - 1, sizeof(int));
