...
__device__ void merge(int *testdata, T1 *dstobject, T2 *srcobject)
{
	for(int index = 0; index<srcobject->num_buckets; index++)
	{
		if((srcobject->buckets)[index]!=0)		
		{
			...
			//atomicAdd(testdata, value_size);
			void *key = srcobject->get_key_address(index);	
			void *value = srcobject->get_value_address(index);
			dstobject->insert_or_update_value(testdata, key, key_size, value, value_size);
		}
	}
}
...
void *global_offset, unsigned int offset_number, unsigned int unit_size)
{
	...
	while(finished!=NUM_THREADS)
	{
		for(; i < offset_number; i+= num_threads)		
			...
			#ifdef MERGE_WHEN_FULL
			bool success = Map_Reduce::map(testdata, &objects[gid], object_g, global_data, ((char *)global_offset+unit_size*i));
			#endif
			
			#ifdef SORT_WHEN_FULL
			bool success = Map_Reduce::map(testdata, &objects[use_index][gid], object_g, global_data, ((char *)global_offset+unit_size*i));
			#endif
		...
		#ifdef MERGE_WHEN_FULL
		merge_to_global(object_g, &objects[gid]);

	//	if(tid%group_size==0)
	//	{
	//		atomicAdd(testdata, 1);
	//		//merge(testdata, object_g, &objects[gid]);
	...
	Map_Reduce::map(testdata, object_g, object_g, global_data, ((char *)global_offset+unit_size*i));			
	#endif
} 
