...
//__device__ Reduction_Object* newReduction_Object()
//{
//	unsigned short object_offset = smalloc(sizeof(Reduction_Object));
//	Reduction_Object *newObject = (Reduction_Object*)get_address(object_offset);	
//	newObject->oma_init();
//	return newObject;
//}
	...
	while(DoWork)
	{
		...
		if(getLock(&locks[index]))
		{
			if(buckets[index]==0)
			{
				...
				short k  = omalloc(1+key_size);//The first byte stores size of the key
